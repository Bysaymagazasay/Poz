(() => {
  ...
  (async()=>{
    ...
    try{if(window.BYSAY_LEGACY_READY)await window.BYSAY_LEGACY_READY;if(typeof window.BYSAY_APPLY_LEGACY_CACHE==='function')window.BYSAY_APPLY_LEGACY_CACHE();}catch(error){console.error(error);window.BYSAY_LEGACY_LOAD_ERROR=error?.message||String(error);}
    ...
    const errors=[];if(window.BYSAY_DATA_LOAD_ERROR)errors.push(`İnşaat fiyat listesi: ${window.BYSAY_DATA_LOAD_ERROR}`);if(window.BYSAY_USER_BOOK_ERROR)errors.push(`Kayıtlı poz kitapları: ${window.BYSAY_USER_BOOK_ERROR}`);if(window.BYSAY_ELECTRIC_LOAD_ERROR)errors.push(`Elektrik fiyat listesi: ${window.BYSAY_ELECTRIC_LOAD_ERROR}`);if(window.BYSAY_LEGACY_LOAD_ERROR)errors.push(`Eski poz güncelleme: ${window.BYSAY_LEGACY_LOAD_ERROR}`);if(window.BYSAY_HIDDEN_BOOK_ERROR)errors.push(`Poz kitabı görünürlüğü: ${window.BYSAY_HIDDEN_BOOK_ERROR}`);
